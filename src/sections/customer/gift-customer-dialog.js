...
import { Button, Dialog, DialogActions, DialogContent, DialogTitle, FormHelperText, IconButton, Input, InputLabel, MenuItem, Select, TextField } from '@mui/material';
...
export const GiftDialog = (props) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object().shape({
      ...
      ticketAmount: Yup.number().min(1, 'A quantidade mínima de tickets é 1').required('Quantidade de Tickets é obrigatório')
    }),
  ...
  return (
    <Dialog open={dialog == 'gift'} maxWidth='sm' fullWidth='fullWidth'>
